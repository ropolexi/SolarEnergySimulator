            ...
            if (extraData.Type == "solar-panel") {
                ...
                if (count > current_count) {
                    for (const i = 0; i < count - current_count; i++) {
                        ...
                        userSimulation[userId].solarPanels.push({ id: newId, maxPower: parseInt(extraData.maxPower), power: 0 })
                    }
                ...
                if (count > current_count) {
                    for (const i = 0; i < count - current_count; i++) {
                        ...
                        userSimulation[userId].batteries.push({ id: newId, capacity: parseInt(extraData.capacity), charge: parseInt(extraData.capacity) / 2 })
                    }
                } else if (count < current_count) {
                    for (const i = 0; i < current_count - count; i++) {
                ...
                if (count > current_count) {
                    for (const i = 0; i < count - current_count; i++) {
                        ...
                        userSimulation[userId].inverters.push({ id: newId, maxPower: parseInt(extraData.maxPower),power:0,on:false,switch:false})
                    }
                } else if (count < current_count) {
                    for (const i = 0; i < current_count - count; i++) {
                ...
                if (count > current_count) {
                    for (const i = 0; i < count - current_count; i++) {
                        ...
                        userSimulation[userId].loads.push({ id: newId, power: parseInt(extraData.power),on:false,switch:true})
                    }
                } else if (count < current_count) {
                    for (const i = 0; i < current_count - count; i++) {
